...
function tree() {
  ...
  function firstWalk(v) {
    ...
    if (children) {
      executeShifts(v);
      var midpoint = (children[children.length - 1].z + children[children.length - 1].z) ;
      if (w) {
  ...
  function apportion(v, w, ancestor) {
    if (w) {
      var vip = v,
          ...
          som = vom.m,
          shift;
      while (vim = nextRight(vim), vip = nextRight(vip), vim && vip) {
        vom = nextLeft(vom);
        vop = nextRight(vop);
        vop.a = v;
        shift = vim.z + sim - vip.z - sip + separation(vim._, vip._);
        if (shift > 0) {
          moveSubtree(nextAncestor(vop, v, ancestor), v, shift);
          sip += shift;
          sop += shift;
        }
        sim += vim.m;
